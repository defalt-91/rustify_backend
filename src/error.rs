    ...
    fn into_response(self) -> Response {
        match self {
            ...
            UserError::NotAuthenticated => {
                (StatusCode::FORBIDDEN, headers(), "You are not authorized")
            }
            UserError::UserAgentMissing => (
                StatusCode::BAD_REQUEST,
                headers(),
                "User-Agent header is missing",
            ),
            UserError::AuthorizationHeaderMissing => (
                StatusCode::UNAUTHORIZED,
                headers(),
                "Authorization header is missing",
            ),
            UserError::AuthorizationHeaderFormatWrong => (
                StatusCode::FORBIDDEN,
                headers(),
                "Authorization header format is wrong",
            ),
            UserError::AuthorizationHeaderEmpty => (
                StatusCode::FORBIDDEN,
                headers(),
                "Empty header is not allowed",
            ),
            UserError::WrongToken => (
                StatusCode::UNAUTHORIZED,
                headers(),
