...
pub async fn wg_dump(
    ...
    let dump_output = sudo_exec(&ctx, vec!["wg", "show", "wg0", "dump"])
        .await
...
pub async fn wg_rxtx_lha(ctx: Ctx) -> ApiResult<Json<Vec<PeerRxTxDump>>> {
    let t_output = sudo_exec(&ctx, vec!["wg", "show", "wg0", "transfer"])
        .await
        .map_err(ApiError::from(&ctx))?;
    ...
    let lha_output = sudo_exec(&ctx, vec!["wg", "show", "wg0", "latest-handshakes"])
        .await
        .map_err(ApiError::from(&ctx))?;
