...
pub struct IfId { interface_id:Option<usize> }
pub async fn wg_dump(
    Query(interface_id):Query<IfId>,
    ctx: Ctx,
    ...
    let dump_vec: Vec<&str> = dump.strip_suffix("\n").map(|v| v.split("\n").collect())
        .ok_or(ApiError { req_id: ctx.req_id(), error: BaseError::PeerDumpError("wg show dump error") })?;
...
pub async fn wg_rxtx_lha(ctx: Ctx) -> ApiResult<Json<Vec<PeerRxTxDump>>> {
    ...
    let dump_transfer = t_string.strip_suffix("\n").map(|v| v.split("\n"))
        .ok_or(ApiError { req_id: ctx.req_id(), error: BaseError::PeerDumpError("wg show transfer error") })?;
    ...
    let dump_last_handshake = lha_string.strip_suffix("\n").map(|v| v.split("\n"))
        .ok_or(ApiError { req_id: ctx.req_id(), error: BaseError::PeerDumpError("latest-handshakes") })?;
    Ok(dump_transfer.zip(dump_last_handshake).map(|v| PeerRxTxDump::from_rxtx_lha(v)).collect())
        .map(Json)
}
